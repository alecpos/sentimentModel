...
class CustomTensorBoardCallback:
    ...
    def on_epoch_begin(self, args, state, control, **kwargs):
        ...
        if self.writer:
            self.writer.add_text("epoch", f"Starting epoch {state.epoch}")
    
    def on_epoch_end(self, args, state, control, **kwargs):
        """Called when an epoch ends."""
        if self.writer:
            self.writer.add_text("epoch", f"Completed epoch {state.epoch}")
...
class TransformerSentimentAnalyzer:
    ...
    def __init__(self, model_type='bertweet', num_labels=2):
        ...
        if model_type not in self.model_mapping:
            raise ValueError(f"Invalid model type: {model_type}. Must be one of {list(self.model_mapping.keys())}")
            ...
            if hasattr(self.model, 'classifier'):
                ...
                try:
                    ...
                    # Pattern 3: BERT-style classifier with pre_classifier
                        ...
                        torch.nn.init.zeros_(self.model.classifier.pre_classifier.bias)
                        torch.nn.init.kaiming_normal_(self.model.classifier.weight, mode='fan_out', nonlinearity='relu')
                        torch.nn.init.zeros_(self.model.classifier.bias)
    ...
    def train(self, train_dataset, val_dataset, args):
        ...
        base_args = {
            ...
            'fp16': not is_a100,  # Use fp16 for non-A100 GPUs
            'bf16': is_a100,      # Use bf16 for A100 GPUs
        ...
        # Initialize trainer with only EarlyStoppingCallback
        callbacks = [
    ...
    def predict(self, text: str, return_confidence: bool = False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        Returns:
            Prediction (0 or 1) or tuple of (prediction, confidence scores)
    ...
    def save_model(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, save_path: str) -> None:
    ...
    def _preprocess_labels(self, labels: np.ndarray) -> np.ndarray:
        ...
        Args:
            labels: Input labels to preprocess
    ...
    def _evaluate_model(self, eval_dataset):
    ...
    def _evaluate_fairness(self, texts, labels, demographics):
        """Evaluate model fairness across demographic groups.
        
        Args:
            texts: List of input texts
            labels: List of true labels
            demographics: DataFrame containing demographic information
        ...
        demographic_parity = {}
        for group in demographics.columns:
            group_predictions = predictions[demographics[group] == 1]
            demographic_parity[group] = np.mean(group_predictions)
        
        # Calculate equal opportunity
        equal_opportunity = {}
        for group in demographics.columns:
            group_predictions = predictions[(demographics[group] == 1) & (labels == 1)]
            equal_opportunity[group] = np.mean(group_predictions)
    ...
    def _apply_bias_mitigation(self, texts, labels, demographics):
        """Apply bias mitigation techniques to the model.
        ...
        Returns:
            None
        """
        # This is a placeholder for bias mitigation techniques
        # In practice, you would implement techniques like:
        # - Adversarial debiasing
        # - Counterfactual data augmentation
        # - Fair representation learning
        pass

    def _ensemble_predict(self, texts, models=None):
        """Make ensemble predictions using multiple models.
        
        Args:
            texts: List of input texts
            models: List of additional models to use for ensemble
        ...
        predictions = [self.predict_batch(texts)]
        for model in models:
            predictions.append(model.predict_batch(texts))
...
class EnsembleSentimentAnalyzer:
    ...
    def _train_bagging_models(self, X_train, y_train, X_val, y_val):
        ...
        X_train = np.array(X_train.toarray() if hasattr(X_train, 'toarray') else X_train)
        y_train = np.array(y_train)
        X_val = np.array(X_val.toarray() if hasattr(X_val, 'toarray') else X_val)
    ...
    def _prepare_background_dataset(self, train_dataset, n_samples=100):
        ...
        try:
            ...
            logger.info("Processing full dataset for background sampling...")
            all_texts = []
            processed_count = 0
            ...
            for i in range(0, len(train_dataset), batch_size):
                try:
                    ...
                    if processed_count % 10000 == 0:
                        logger.info(f"Processed {processed_count}/{len(train_dataset)} samples...")
            ...
            logger.info(f"Successfully extracted {len(all_texts)} texts from full dataset")
            
            # Fit TF-IDF on the full dataset
            logger.info("Fitting TF-IDF vectorizer...")
            try:
                self.tfidf.max_features = min(5000, len(all_texts))
            ...
            # Use MiniBatchKMeans with larger batch size for full dataset
            ...
            try:
                ...
                # Fit in larger batches
                batch_size = min(5000, X.shape[0])
                for i in range(0, X.shape[0], batch_size):
                    try:
                        end_idx = min(i + batch_size, X.shape[0])
                        kmeans.partial_fit(X[i:end_idx].toarray())
                    except Exception as e:
                        logger.error(f"Error during clustering batch {i}: {str(e)}")
                        continue
                
                # Get representative samples
                logger.info("Finding representative samples...")
                try:
                    distances = kmeans.transform(X.toarray())
                    background_indices = []
                    ...
                    for cluster_idx in range(kmeans.n_clusters):
                        try:
                            ...
                            logger.error(f"Error processing cluster {cluster_idx}: {str(e)}")
                            continue
                    ...
                    logger.error(f"Error finding representative samples: {str(e)}")
                    raise
            ...
            logger.error("All attempts to create background dataset failed")
            return False
    ...
    def explain_prediction(self, text: str, cache: bool = True) -> Dict[str, Any]:
        ...
        explanation = {
            ...
            'prediction_proba': self.predict([text], return_confidence=True)[0]
        }
    ...
    def _plot_token_contributions(self, transformer_explanation: Dict[str, Any], ax):
        ...
        mask = [t not in ['<pad>', ' ', ' '] for t in tokens]
    ...
    try:
        ...
        class Args:
            def __init__(self):
                ...
                self.max_samples = None  # Set to None to use full dataset
                self.model_type = 'bertweet'
                self.test_size = 0.1
                self.batch_size = 64  # Reduced batch size for better memory management
                ...
                self.gradient_accumulation_steps = 8  # Added for better memory management
        ...
        # Load full dataset
        df = load_sentiment140(max_samples=None)
        ...
        # Create datasets with memory-efficient processing
        ...
        # Train ensemble model with memory optimizations
